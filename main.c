...
uchar datatime = 0; //保存数据时间
uchar testnum = 1; //测试步骤
uchar testtime = 0; //测试时间
uchar isok = 0; //测试通过标志

#define TIMEOUT  15//操作超时时间
uchar timeout = TIMEOUT;

#define KEYTIME  10//按键间隔时间*10ms
uchar keytime = 0;
...
void saveData() {
    ...
    if (checksum(databuf, 15) != databuf[15]) err = EMEM;
    else {
        if (err == EMEM) err = ENIL;
    }
}
...
void initData() {
    uchar i;
    for (i = 0; i < 16; i++) databuf[i] = at93c46Read(0x20 + i);
    if (checksum(databuf, 15) == databuf[15]) {
...
ISR(TIMER0_OVF_vect) {
    ...
    if (time0num % 100 == 0) testtime++; //测试时间
}
//温度湿度测量
...
void login() {
        ...
        case OK:
        {
            ...
            if (editnum >= 3) {
                ...
                if (checksum(pass, 2) == pass[2]) {
                    if (pass[0] == (editbuf[0]*16 + editbuf[1]) && pass[1] == (editbuf[2]*16 + editbuf[3])) {
                        islogin = 1;
                        isadmin = 1;
                    }
                } else {
                    if (editbuf[0] * pow(2, 12) + editbuf[1] * pow(2, 8) + editbuf[2] * pow(2, 4) + editbuf[3] == PASS) {
                        islogin = 1;
                        isadmin = 1;
                    }
                }
    ...
    if (isclr) {
        ...
        editnum = 0;
        editbuf[0] = 0;
        oledZh(24, 0, 31, 3, ch); //请输入
        oledZh(72, 0, 34, 2, ch); //密码
    }
    for (i = 0; i < 4; i++) {
        if (i < editnum) oledChar(36 + 16 * i, 3, 5, 1, symbol); //*
        if (i == editnum) oledCharInverse(36 + 16 * i, 3, editbuf[i], 1, digit); //0
        if (i > editnum) oledChar(36 + 16 * i, 3, 4, 1, symbol); //_
    }
}
//新风启动温度
...
void setpass(uchar admin) {
    ...
    switch (key) {
        ...
        case OK:
        {
            key = NIL;
            if (editnum >= 7) {
                if (editbuf[0] == editbuf[4] && editbuf[1] == editbuf[5] && editbuf[2] == editbuf[6] && editbuf[3] == editbuf[7]) {
                    confbuf[0] = editbuf[0]*16 + editbuf[1];
                    confbuf[1] = editbuf[2]*16 + editbuf[3];
                    confbuf[2] = checksum(confbuf, 2);
                    if (admin) passaddr = 0x13;
                    else passaddr = 0x10;
                    for (i = 0; i < 3; i++) at93c46Write(passaddr + i, confbuf[i]);
                    for (i = 0; i < 3; i++) confbuf[i] = at93c46Read(passaddr + i);
                    if (checksum(confbuf, 2) == confbuf[2]) {
                        oledZh(64, 6, 89, 2, ch); //成功
                    } else {
                        oledZh(64, 6, 91, 2, ch); //失败
                    }
                    oledZh(32, 6, 36, 2, ch); //修改
                    _delay_ms(1000);
                    page = MENU;
                    isclr = 1;
                    return;
                } else {
                    oledZh(8, 6, 39, 2, ch); //两次
                    oledZh(40, 6, 34, 2, ch); //密码
                    oledZh(72, 6, 41, 3, ch); //不一致
                    _delay_ms(1000);
                    isclr = 1;
                }
            } else {
    ...
    if (isclr) {
        ...
        editnum = 0;
        editbuf[0] = 0;
        oledZh(8, 0, 36, 2, ch); //修改
        if (admin) oledZh(40, 0, 83, 2, ch); //管理
        else oledZh(40, 0, 80, 2, ch); //操作
        oledZh(72, 0, 82, 1, ch); //员
        oledZh(88, 0, 34, 2, ch); //密码
        oledZh(16, 2, 104, 1, ch); //新
        oledZh(32, 2, 34, 2, ch); //密码
        oledChar(64, 2, 1, 1, symbol); //:
        oledZh(0, 4, 105, 2, ch); //确认
        oledZh(32, 4, 34, 2, ch); //密码
        oledChar(64, 4, 1, 1, symbol); //:
    }
    y = 2;
    for (i = 0; i < 8; i++) {
        x = 74 + 15 * i;
        ...
        if (i < editnum) oledChar(x, y, 5, 1, symbol); //*
        if (i == editnum) oledCharInverse(x, y, editbuf[i], 1, digit);
        if (i > editnum) oledChar(x, y, 4, 1, symbol); //_
    }
}
//清零系统数据
    ...
    switch (testnum) {
        ...
        {//内传感器
            if (isclr) {
                ...
                testtime = 0;
                isok = 0;
                oledChar(0, 0, testnum, 1, digit);
                oledChar(8, 0, 2, 1, symbol);
                oledZh(16, 0, 107, 1, ch); //室
                oledZh(32, 0, 0, 1, ch); //内
                oledZh(48, 0, 16, 2, ch); //温度
                oledZh(80, 0, 113, 3, ch); //传感器
                oledZh(32, 6, 74, 2, ch); //测试
                oledZh(16, 2, 16, 2, ch); //温度
                oledChar(48, 2, 1, 1, symbol); //:
                oledZh(96, 2, 95, 1, ch); //℃
                oledZh(16, 4, 4, 1, ch); //湿
                oledZh(32, 4, 17, 1, ch); //度
                oledChar(48, 4, 1, 1, symbol); //:
                oledChar(80, 4, 6, 1, symbol); //%
                oledZh(64, 6, 28, 1, ch); //中
                oledZh(80, 6, 119, 1, ch); //...
            }
            DecBcd(item, disbuf, 4);
            if (item < 0) {
                oledChar(56, 2, 0, 1, symbol);
            } else {
                if (disbuf[3] > 0) oledChar(56, 2, disbuf[3], 1, digit);
                else oledChar(56, 2, 3, 1, symbol);
            }
            oledChar(64, 2, disbuf[2], 1, digit);
            oledChar(72, 2, disbuf[1], 1, digit);
            oledChar(80, 2, 2, 1, symbol);
            oledChar(88, 2, disbuf[0], 1, digit);
            oledChar(64, 4, ihum / 10, 1, digit);
            oledChar(72, 4, ihum % 10, 1, digit);
            if (item != 0 && ihum != 0) {
                if(!isok){
                    isok = 1;
                    testtime = 0;
                    oledZh(64, 6, 117, 2, ch); //通过
                }
            }else{
                if(testtime >= 10){
                    oledZh(64, 6, 91, 2, ch); //失败
        ...
        {//外传感器
            ...
            oledChar(80, 2, 2, 1, symbol);
            oledChar(88, 2, disbuf[0], 1, digit);
            oledChar(64, 4, ohum / 10, 1, digit);
            oledChar(72, 4, ohum % 10, 1, digit);
            if (otem != 0 && ohum != 0) {
                if(!isok){
                    isok = 1;
                    testtime = 0;
        ...
        case 3: //空调A相
        {
            if (isclr) {
                ...
                oledChar(8, 0, 2, 1, symbol);
                oledZh(16, 0, 5, 2, ch); //空调
                oledZh(48, 0, 49, 1, ch); //电
                oledZh(64, 0, 111, 1, ch); //压
                oledZh(80, 0, 49, 1, ch); //电
                oledZh(96, 0, 112, 1, ch); //流
                oledChar(112, 0, 10, 1, digit);//A
                oledZh(16, 2, 49, 1, ch); //电
                oledZh(32, 2, 111, 1, ch); //压
                oledChar(48, 2, 1, 1, symbol); //:
                oledZh(16, 4, 49, 1, ch); //电
                oledZh(32, 4, 112, 1, ch); //流
                oledChar(48, 4, 1, 1, symbol); //:
                oledZh(32, 6, 74, 2, ch); //测试
                oledZh(64, 6, 28, 1, ch); //中
                oledZh(80, 6, 119, 1, ch); //...
            ...
            DecBcd(ua, disbuf, 4);
            oledChar(64, 2, disbuf[3], 1, digit);
            oledChar(72, 2, disbuf[2], 1, digit);
            oledChar(80, 2, disbuf[1], 1, digit);
            oledChar(88, 2, 2, 1, symbol); //.
            oledChar(96, 2, disbuf[0], 1, digit);
            oledChar(104, 2, 0, 1, letter); //V
            DecBcd(ia, disbuf, 5);
            if (disbuf[4] > 0) oledChar(56, 4, disbuf[4], 1, digit);
            else oledChar(56, 4, 3, 1, symbol);
            oledChar(64, 4, disbuf[3], 1, digit);
            oledChar(72, 4, 2, 1, symbol); //.
            oledChar(80, 4, disbuf[2], 1, digit);
            oledChar(88, 4, disbuf[1], 1, digit);
            oledChar(96, 4, disbuf[0], 1, digit);
            oledChar(104, 4, 10, 1, digit); //A
            if (ua > 10*180 && ia > 1000*0.01) {
                if(!isok){
                    isok = 1;
                    testtime = 0;
                    oledZh(64, 6, 117, 2, ch); //通过
        ...
        case 6: //空调开
        {
            if (isclr) {
                ...
                oledZh(16, 0, 5, 2, ch); //空调
                oledZh(48, 0, 9, 2, ch); //开关
                oledZh(80, 0, 74, 2, ch); //测试
                oledZh(28, 3, 57, 2, ch); //是否
                oledZh(76, 3, 9, 1, ch); //开
                oledZh(60, 3, 121, 1, ch); //已
                oledChar(92, 3, 7, 1, symbol);//?
                airAction(1);
            }
            if(isok){
                oledZh(32, 6, 74, 2, ch); //测试
        ...
        case 8: //进风机
        {
            if (isclr) {
                ...
                oledChar(0, 0, testnum, 1, digit);
                oledChar(8, 0, 2, 1, symbol);
                oledZh(16, 0, 122, 2, ch); //进
                oledZh(32, 0, 8, 2, ch); //风
                oledZh(48, 0, 101, 2, ch); //机
                oledZh(64, 0, 9, 2, ch); //开关
                oledZh(96, 0, 74, 2, ch); //测试
                oledZh(28, 3, 57, 2, ch); //是否
                oledZh(76, 3, 9, 1, ch); //开
                oledZh(60, 3, 121, 1, ch); //已
                oledChar(92, 3, 7, 1, symbol);//?
        ...
        default: //测试完成
        {
            if (isclr) {
                ...
                isclr = 0;
                isok = 0;
                oledZh(32, 3, 74, 2, ch); //测试
                oledZh(64, 3, 123, 2, ch); //完成
                windAction(0);
            }
            break;
        }
    }
    if(isok&&testtime>=2){ //自动跳转下一步
        testnum++;
        isclr = 1;
    }
}
